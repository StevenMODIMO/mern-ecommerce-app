...
export default function UserDashboard() {
  ...
  const removeFromCart = async (id) => {
    const response = await fetch(`http://localhost:5000/api/app/remove-from-cart/${id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${user.token}`
      }
  ...
  const removeFromWishList = async (id) => {
    const response = await fetch(`http://localhost:5000/api/app/remove-wishlist/${id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${user.token}`
      }
  ...
  return (
    <div className="lg:flex">
      <header className="flex w-screen text-lg lg:flex lg:flex-col">
        <div onClick={() => setShowTab(true)} className={showTab ? "w-2/4 bg-green-500 rounded flex justify-center gap-1" : "w-2/4 text-start border-b border-green-500 rounded flex justify-center gap-1"}>
          <AiOutlineShoppingCart className="mt-1" />
        <button>
          Cart
        </button>
        </div>
        <div onClick={() => setShowTab(false)} className={!showTab ? "w-2/4 bg-green-500 rounded flex justify-center gap-1" : "w-2/4 text-start border-b border-green-500 rounded flex justify-center gap-1"}>
            ...
            {orders.map((order) => {
              return (
                ...
                >
                  ...
                  <section onClick={() => removeFromCart(order._id)} className="text-lg flex gap-1 justify-center bg-red-500 p-1 rounded">
            ...
            {wish.map((w) => {
              return (
                ...
                >
                  ...
                  <section onClick={() => removeFromWishList(w._id)} className="text-lg flex gap-1 justify-center bg-red-500 p-1 rounded">
                    <BsCartDash className="mt-1" />
